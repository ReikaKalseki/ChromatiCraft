...
public class StructureCalculator implements Runnable {
	...
	private void generate() throws OutOfMemoryError {
		...
		for (int i = 0; i < DimensionStructureType.types.length; i++) {
			...
			colors.remove(index);
			ChunkProviderChroma.structures.add(new StructurePair(DimensionStructureType.types[i], e.ordinal()));
		}
	...
	private void doGenerate(StructurePair s, int structureOriginX, int structureOriginZ, float structureAngleOrigin) {
		float ang = structureAngleOrigin+s.genIndex*22.5F;
		...
		if (DragonAPICore.isReikasComputer() && ReikaObfuscationHelper.isDeObfEnvironment()) {
			String sg = "CHROMATICRAFT: Generated a #"+s.genIndex+" "+s.generator+" at "+s.generator.getGenerator().getCentralBlockCoords();
