...
void main() {
	...
	float t = float(time)*0.004;
	float noiseVal = snoise4d(vec4(worldCoord*0.05, t));
	...
	float edgeF = 1.0;//pow(getVisualBrightness(getGrayscaledBlurredEdgeDetectedColor(texcoord)), 4);
	float f = max(0.0, 0.06-abs(noiseVal))*30.0*intensity*edgeF;
	//gl_FragColor.rgb += glow*min(1.0, (1.0-pow(br*3, 0.1))*f*3.0);
	gl_FragColor.rgb = max(gl_FragColor.rgb, glow*min(1.0, (1.0-pow(br*3, 0.1))*f*3.0));
}