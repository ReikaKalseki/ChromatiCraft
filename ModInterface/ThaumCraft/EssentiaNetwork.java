	...
	public static class EssentiaPath {
		...
		public static void sendParticle(World world, double x1, double x2, double y1, double y2, double z1, double z2, String asp, int amt) {
			...
			for (double d = 0; d <= dd; d += 0.125) {
				...
				EntityFX fx = new EntityCCBlurFX(world, px, py, pz).setColor(a.getColor()).setLife(l).setScale(s).setRapidExpand();//.setIcon(ChromaIcons.TRANSFADE).setBasicBlend();
				EntityFX fx2 = new EntityCCBlurFX(world, px, py, pz).setColor(0xffffff).setLife(l).setScale(s/2).setRapidExpand();//.setIcon(ChromaIcons.TRANSFADE).setBasicBlend();
				Minecraft.getMinecraft().effectRenderer.addEffect(fx);
				Minecraft.getMinecraft().effectRenderer.addEffect(fx2);
				i++;
			}
