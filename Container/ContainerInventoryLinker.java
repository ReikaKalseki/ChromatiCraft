...
public class ContainerInventoryLinker extends Container {
	...
	public ContainerInventoryLinker(EntityPlayer player, World par2World)
	{
		...
		for (int i = 0; i < height; i++) {
			for (int k = 0; k < width; k++) {
				this.addSlotToContainer(new Slot(craftMatrix, i*width+k, 8+k*18, 17+i*18));
			}
		}
		...
		for (int i = 0; i < li.size(); i++) {
			craftMatrix.setInventorySlotContents(i, li.get(i));
		}
	...
	public void onContainerClosed(EntityPlayer ep) {
		...
		for (int i = 0; i < craftMatrix.getSizeInventory(); i++) {
			ItemStack in = craftMatrix.getStackInSlot(i);
			if (in != null) {
				li.add(in);
			}
		}
